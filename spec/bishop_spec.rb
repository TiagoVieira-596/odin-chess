...
describe Bishop do
  context 'finds all possible moves with an empty board' do
    let(:bishop_empty_moves) { Bishop.new('bishop') }
  ...
  context 'gets blocked by other pieces' do
    let(:bishop_blocked_moves) { Bishop.new('bishop') }
    full_board_black = Array.new(8) { Array.new(8, Bishop.new('bishop')) }
    full_board_white = Array.new(8) { Array.new(8, Bishop.new('white', 'bishop')) }
